...
class PlannedTransactionModal extends React.Component {
  ...
  componentDidMount() {
    window.openPlannedTransactionModal = this.openModal;
    if (this.props.onRegisterModal) {
      this.props.onRegisterModal("planned-transaction", this.openModal);
    }
  }
  ...
  openModal(items) {
    this.setState(() => {
      const data = items.transaction
        ...
        : [PlannedTransactionModal.newRow()];
  ...
  handleRowToData(evt) {
    ...
    this.setState(prevState => ({
      ...
      data: prevState.data.concat(PlannedTransactionModal.newRow(prevState["dataLastKey"] + 1)),
    }));
  }
  ...
  handleRemoveFromList(evt) {
    this.setState(prevState => {
      ...
      return {
        data: [PlannedTransactionModal.newRow()],
        dataLastKey: 0,
      };
    });
  }
  ...
  _renderConfirm() {
    ...
    let title = null;
    if (this.state.id !== 0) {
      action = `/planned-transaction/${this.state.id}/edit`;
      title = "Modifier la transaction planifiée";
    } else {
      if (this.state.type === TransactionType.EXPECTED_INCOME) {
        title = "Créer un revenu planifiée";
      } else {
        title = "Créer une dépense planifiée";
      }
      action = "/planned-transaction/new";
    }
    ...
    >
      ...
      <CategoryModal
        ...
        genre={this.state.type === "EXPECTED_EXPENSE" ? "OUTCOME" : "INCOME"}
      />
    </Modal>;
  }
  ...
  render() {
    return <div>
      ...
      <Modal
        ...
        handleConfirm={PlannedTransactionModal.handleAlertClick}
      >
...
PlannedTransactionModal.displayName = "PlannedTransactionModal";
PlannedTransactionModal.propTypes = {
  /* visible: PropTypes.bool.isRequired,
  transaction: PropTypes.object,
  onClose: PropTypes.func.isRequired,*/
  onRegisterModal: PropTypes.func.isRequired,
};
// PlannedTransactionModal.defaultProps = {transaction: undefined};

module.exports = PlannedTransactionModal;
